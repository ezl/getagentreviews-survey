...
export default {
  ...
  methods: {
    async saveReview () {
      ...
      await this.$store.dispatch('reviews/stepComplete', { star_rating_completed: new Date(), id: this.$store.state.reviews.reviewRequest.id, star_rating: this.$store.state.reviews.chosen, route: this.routeTo + this.$store.state.reviews.reviewRequest.id })
    }
  }
}
</script>
